 ...
 * @copyright 2007-2011
  ...
  **/
	...
	require_once WideImage::path() . 'Images.php';
	...
	class WideImage
	{
		 ...
		 * To load only a single image from an array field, use WideImage::loadFromUpload('images', $i),
		 * where $i is the index of the image you want to load.
		 * 
		 * <code>
		 * $images = WideImage::load('http://url/image.png'); // image URL
		 * $images = WideImage::load('/path/to/image.png'); // local file path
		 * $images = WideImage::load('images'); // upload field name
		 * $images = WideImage::load(imagecreatetruecolor(10, 10)); // a GD resource
		 * $images = WideImage::load($image_data); // binary string containing image data
		 * </code>
		 * 
